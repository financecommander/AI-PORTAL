...
cat > 'backend/config/specialists.json' << 'FILEEOF_backend_config_specialists_json'
{
  "specialists": [
    {
...
FILEEOF_backend_config_specialists_json
echo '  ✅ backend/config/specialists.json'
...
echo ""
git add -A
git commit --no-gpg-sign -m "P6: rename nav — Console, Analyst Desks, Engines, Metrics

Sidebar: Chat→Console, Specialists→Analyst Desks, Pipelines→Engines, Usage→Metrics
LLMChatPage: Direct LLM Access→Intelligence Console
ChatPage: Specialists→Analyst Desks
UsagePage: Usage & Costs→Intelligence Metrics, Chat Logs→Console Logs, Pipeline Runs→Engine Runs
SettingsPage: Intelligence Pipelines→Intelligence Engines" || echo "Nothing"
git push origin main
echo "✅ Pushed. Frontend only:"
